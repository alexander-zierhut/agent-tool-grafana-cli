...
from grafanacli import __version__
...
def test_report_runs_offline_and_names_this_repo(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("GRAFANACLI_TOKEN", raising=False)

    result = CliRunner().invoke(app, ["report"])
    assert result.exit_code == 0, result.output

    data = json.loads(result.stdout)
    assert data["repo"] == "alexander-zierhut/agent-tool-grafana-cli"
    assert data["published"] is True
    assert data["version"] == __version__
    assert data["issues"].endswith("/agent-tool-grafana-cli/issues")
